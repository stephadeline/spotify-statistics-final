...
```{r}
#top 10 genres only
ggplot(top10, aes(x=energy, y=loudness)) + geom_point((aes(colour = track_genre))) + geom_smooth(method=lm) + theme(legend.title = element_blank())+ labs(x = "Energy", y = "Loudness", title = "Association between Energy and Loudness")
```
...
```{r}
ggplot(top10, aes(x=energy, y=acousticness)) + geom_point((aes(colour = track_genre))) + geom_smooth(method=lm) + theme(legend.title = element_blank())+ labs(x = "Energy", y = "Acousticness", title = "Association between Energy and Acousticness")
```
...
Doing a multiple regression also improves the model a bit, with adjusted r-squared jumping from 0.601 to 0.71.

However, upon looking at the residuals and the number of outliers, the models we've created might not be the best at predicting a song's energy levels, and perhaps we should consider another type of model.
