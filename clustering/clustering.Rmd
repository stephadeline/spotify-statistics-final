...
dd_clust <- dd %>% bind_cols(tibble(cluster = c2))

dd_clust %>%
  group_by(cluster, as.factor(track_genre)) %>%
  summarize(n = n()) %>%
  arrange(desc(n))
  
dd_forplot <- dd_clust %>%
  select(track_genre, cluster) %>%
  group_by(cluster, track_genre) %>%
  count() %>%
  ungroup() %>%
  group_by(cluster) %>%
  top_n(10, track_genre) %>
  arrange(cluster, desc(n))
  
ggplot(dd_forplot) +
  geom_col(aes(x = cluster, y = n, fill = track_genre)) +
  geom_text(aes(x = cluster, y = n, label = track_genre))

dd_clust %>%
  select(popularity, track_genre, cluster) %>%
